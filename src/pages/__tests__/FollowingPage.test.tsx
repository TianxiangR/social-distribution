...
describe('FollowingPage', () => {
  ...
  it('should render followers', async () => {
    jest.spyOn(apis, 'getFollowing').mockResolvedValueOnce(
      {
        ok: true,
        status: 200,
        json: async () => authorsData
      } as Response
    );
    ...
    for (const author of authorsData.items) {
      await waitFor(() => screen.getByText(author.displayName));
    }
  });
});